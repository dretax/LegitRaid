...
namespace LegitRaid
{
    public class LegitRaid : Fougerite.Module
    {
        ...
        {
            if (!File.Exists(Path.Combine(ModuleFolder, "Logs.log")))
            {
                File.Create(Path.Combine(ModuleFolder, "Logs.log")).Dispose();
            }
        ...
        public void ReloadConfig()
        {
            try
            {
                if (!File.Exists(PathC))
                {
                    ...
                    Settings.AddSetting("Settings", "DataStoreTables",
                        "ExamplePluginDataStoreName,ExamplePluginDataStoreName2,ExamplePluginDataStoreName3");
                    Settings.AddSetting("Settings", "WhiteListedIDs",
                        "76561197967414xxx,76561197961635xxx,76561197961634xxx");
            ...
            {
                Logger.LogError("[LegitRaid] Error when loading config: " + ex);
            }
        }
        ...
        public void OnModulesLoaded()
        {
            ...
            {
                foreach (object x in instance.Keys("LRaiderTime"))
                {
                    object value = instance.Get("LRaiderTime", x);
                    if (x is ulong uid && value is int valint)
                    {
                        RaiderTime[uid] = valint;
                    }
                }
            }
            ...
            if (instance.GetTable("LOwnerTimeData") != null)
            {
                ...
                {
                    object value = instance.Get("LOwnerTimeData", x);
                    if (x is ulong uid && value is double valint)
                    {
                        OwnerTimeData[uid] = valint;
                    }
                }
            }
        ...
        public void OnEntityDestroyed(DestroyEvent de)
        {
            if (de.Attacker != null && de.Entity != null && !de.IsDecay)
            {
                ...
                if ((de.WeaponName.ToLower().Contains("explosive") || de.WeaponName.ToLower().Contains("grenade")
                                                                   || de.WeaponName.ToLower().Contains("hatchet") ||
                                                                   de.WeaponName.ToLower().Contains("axe")
                                                                   || de.WeaponName.ToLower().Contains("rock")) &&
                    (de.Entity.Name.ToLower().Contains("wall")
        ...
        public void OnLootUse(LootStartEvent lootstartevent)
        {
            ...
            if (OwnerTimeData.ContainsKey(lootstartevent.Entity.UOwnerID))
            {
                ...
                if (double.IsNaN(calc) || double.IsNaN(ticks))
                {
                    ...
                    lootstartevent.Player.Notice("",
                        "You need to use C4/Grenade on wall and raid within " + RaidTime + " mins!", 8f);
                    lootstartevent.Player.MessageFrom("LegitRaid",
                        orange + "If your friend owns the chest tell him to add you with /addfriend name");
                    lootstartevent.Player.MessageFrom("LegitRaid",
                        orange + "After that tell him to type /friendraid !");
                ...
                if (DataStore.GetInstance().Get("LegitRaidED", id) != null)
                {
                    ...
                    if (list.Contains(lootstartevent.Entity.OwnerID) && OwnerTimeData.ContainsKey(id))
                    {
                        ...
                        else
                        {
                            var done = Math.Round(calc);
                            lootstartevent.Player.Notice(
                                "You can loot until: " + ((timeraid * 60) - done) + " seconds!");
                        }
